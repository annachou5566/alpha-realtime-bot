...
function hardenCompetitionConfigSignalSource(input) {
    ...
    const setup = [
        ...
        "        const totalDeltaFromCanonical = accumulatedTotal !== null && canonicalAccumulatedTotal !== null",
        "            ? Math.max(0, accumulatedTotal - canonicalAccumulatedTotal)",
        "            : null;",
        "        const limitDeltaFromCanonical = accumulatedLimit !== null && canonicalAccumulatedLimit !== null",
        "            ? Math.max(0, accumulatedLimit - canonicalAccumulatedLimit)",
        "            : null;",
        "        const prospectiveOnchainDelta = totalDeltaFromCanonical !== null && limitDeltaFromCanonical !== null",
        "            ? Math.max(0, totalDeltaFromCanonical - limitDeltaFromCanonical)",
        "            : null;",
        "        const accumulatedOnchain = canonicalAccumulatedOnchain !== null",
        "            ? canonicalAccumulatedOnchain + (prospectiveOnchainDelta === null ? 0 : prospectiveOnchainDelta)",
        "            : (accumulatedTotal !== null && accumulatedLimit !== null ? Math.max(0, accumulatedTotal - accumulatedLimit) : null);",
