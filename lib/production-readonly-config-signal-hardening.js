...
function hardenCompetitionConfigSignalSource(input) {
    ...
    const setup = [
        ...
        "        const rawDailyTotal = waveFiniteLiveNumber(",
        "            live.effectiveTodayVol !== undefined ? live.effectiveTodayVol : live.v && live.v.dt",
        "        );",
        "        const rawDailyLimit = waveFiniteLiveNumber(live.v && live.v.dl);",
        "        const dailyTotal = rawDailyTotal !== null && rawDailyLimit !== null",
        "            ? Math.max(rawDailyTotal, rawDailyLimit)",
        "            : rawDailyTotal;",
        "        const dailyLimit = rawDailyLimit;",
        "        const canonicalDailyTotal = waveFiniteLiveNumber(config && config.real_alpha_volume);",
        "        const canonicalDailyLimit = waveFiniteLiveNumber(config && config.limit_daily_volume);",
        ...
        "        const canonicalAccumulatedOnchain = waveFiniteLiveNumber(config && config.onchain_accumulated_volume);",
        "        const rawTotalDelta = dailyTotal !== null && canonicalDailyTotal !== null",
        "            ? Math.max(0, dailyTotal - canonicalDailyTotal)",
        "            : null;",
        "        const rawLimitDelta = dailyLimit !== null && canonicalDailyLimit !== null",
        "            ? Math.max(0, dailyLimit - canonicalDailyLimit)",
        "            : null;",
        "        const prospectiveTotalDelta = rawTotalDelta !== null && rawLimitDelta !== null",
        "            ? Math.max(rawTotalDelta, rawLimitDelta)",
        "            : null;",
        "        const prospectiveLimitDelta = rawLimitDelta;",
        "        const prospectiveOnchainDelta = prospectiveTotalDelta !== null && prospectiveLimitDelta !== null",
        "            ? Math.max(0, prospectiveTotalDelta - prospectiveLimitDelta)",
        "            : null;",
        "        const accumulatedTotal = canonicalAccumulatedTotal !== null && prospectiveTotalDelta !== null",
        "            ? canonicalAccumulatedTotal + prospectiveTotalDelta",
        "            : null;",
        "        const accumulatedLimit = canonicalAccumulatedLimit !== null && prospectiveLimitDelta !== null",
        "            ? canonicalAccumulatedLimit + prospectiveLimitDelta",
        "            : null;",
        "        const accumulatedOnchain = canonicalAccumulatedOnchain !== null && prospectiveOnchainDelta !== null",
        "            ? canonicalAccumulatedOnchain + prospectiveOnchainDelta",
        "            : null;",
        "        const cumulativeReady = accumulatedTotal !== null && accumulatedLimit !== null && accumulatedOnchain !== null && accumulatedLimit <= accumulatedTotal;",
        ...
        "            item.accumulatedOnchain = accumulatedOnchain;",
        "        }",
