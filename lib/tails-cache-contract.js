...
function validateTailsPayload(data, nowMs = Date.now()) {
    ...
    const actualTotalHash = stableIdsHash(Object.keys(data.total));
    const actualLimitHash = stableIdsHash(Object.keys(data.limit));
    if (hashFields[0] !== actualTotalHash || hashFields[1] !== actualTotalHash) {
        return { ok: false, reason: 'total-keys-hash', expectedBoundary };
    }
    if (hashFields[2] !== actualLimitHash || hashFields[3] !== actualLimitHash) {
