...
function canonicalMachineMessage({ timestamp, method, pathname, body = '' }) {
    const bodyHash = digestHex(String(body));
    return [
        'wave-machine-hmac-v1',
...
function createAlphaLivePublisher(options = {}) {
    ...
    async function publishOnce() {
        ...
        const signature = hmacHex(key, canonical);
