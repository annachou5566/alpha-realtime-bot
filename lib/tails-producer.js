...
function buildCohort(rawTokens, statusMap) {
    ...
    const cohort = [];
    const seen = new Set();
    const ambiguousMissingStatus = [];
    let liveOnlineAcceptedWithoutCache = 0;
    ...
    for (const token of rawTokens) {
        ...
        const offline = token.offline === true;
        const listingCex = token.listingCex === true;
        const cachedStatus = statusMap.get(id);

        // First-party Binance live state is authoritative for currently-online
        // Alpha rows. This exactly mirrors fetch_alpha.py: status defaults to
        // ALPHA and only enters SPOT/PRE_DELISTED handling when offline=true.
        // A stale market-data cache must therefore not make a new online token
        // disappear from the tails cohort.
        if (!offline) {
            if (!contract || chainId === null || chainId === undefined || chainId === '') {
                throw new Error(`active-token-missing-identity:${id}`);
            }
            if (!statusMap.has(id)) liveOnlineAcceptedWithoutCache += 1;
            cohort.push({
                alphaId: id,
                ...
                statusSource: statusMap.has(id) ? 'binance-live+cache' : 'binance-live',
            });
            continue;
        }

        // Offline + listingCex is SPOT by the existing producer contract.
        if (listingCex) continue;

        // Offline non-CEX rows require a known canonical cached classification;
        // missing status here is genuinely ambiguous and must fail closed.
        if (!statusMap.has(id)) {
            ambiguousMissingStatus.push(id);
            continue;
        }
        if (!['ALPHA', 'PRE_DELISTED'].includes(cachedStatus)) continue;
        ...
        cohort.push({
            ...
            chainId: String(chainId),
            volume24h: Number(token.volume24h || 0),
            statusSource: 'cache-offline',
        });
    }

    if (ambiguousMissingStatus.length > 0) {
        throw new Error(`market-status-ambiguous-offline:${ambiguousMissingStatus.length}`);
    }
    if (cohort.length === 0) throw new Error('active-tail-cohort-empty');

    Object.defineProperty(cohort, 'diagnostics', {
        value: { liveOnlineAcceptedWithoutCache },
        enumerable: false,
    });
    return cohort;
}
...
async function runTailsProducer(options) {
    ...
    logger.log(
        ...
        + `live_without_cache=${Number(fullCohort.diagnostics?.liveOnlineAcceptedWithoutCache || 0)}`,
    );
    ...
    return {
        ...
        liveOnlineAcceptedWithoutCache: Number(fullCohort.diagnostics?.liveOnlineAcceptedWithoutCache || 0),
