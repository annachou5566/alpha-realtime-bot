...
function cleanContract(token) {
    const address = String(token.contractAddress);
    // Lowercase only canonical EVM hex addresses. Non-EVM addresses (e.g.
    // TRON/Base58 such as CT_195) are case-sensitive and must be preserved.
...
function parseKlinePayload(payload, tokenId, dataType) {
    ...
    // Binance current offline cohort commonly returns -5095 "bad token address".
    // This is a terminal non-alive signal only for the dedicated offline
    // liveness probe. Active tail fetches must still fail closed on it.
    if (code === '-5095') {
        return {
            capability: 'invalid-address',
            code,
            rows: [],
        };
    }
...
async function requestParsedKline(http, url, tokenId, dataType, options = {}) {
    const {
        metrics = createHttpMetrics(),
        contractAttempts = 2,
        contractRetryDelayMs = 300,
    } = options;

    let lastError = null;
    for (let attempt = 0; attempt < contractAttempts; attempt += 1) {
        const payload = await requestJson(http, url, options);
        try {
            return parseKlinePayload(payload, tokenId, dataType);
        } catch (error) {
            const message = String(error && error.message || error);
            const retryableContract = (
                message.startsWith('kline-payload-type:')
                || message.startsWith('kline-rows-contract:')
            );
            if (!retryableContract) throw error;

            lastError = error;
            if (attempt < contractAttempts - 1) {
                metrics.contractRetries += 1;
                await sleep(contractRetryDelayMs);
                continue;
            }
            throw error;
        }
    ...
    throw lastError || new Error(`kline-contract-retry-exhausted:${tokenId}:${dataType}`);
}
...
async function fetchRecentLimitLiveness(http, token, options) {
    ...
    if (parsed.capability === 'invalid-address') {
        return { alive: false, reason: 'bad-token-address' };
    }
...
async function resolveLiveCohort(http, rawTokens, options = {}) {
    ...
    Object.defineProperty(cohort, 'diagnostics', {
        value: {
            ...
            offlineInvalidAddress: probeResults.filter(
                result => result.reason === 'bad-token-address',
            ).length,
...
async function fetchFullDayKlines(http, token, dataType, boundary, options) {
    ...
    for (let guard = 0; guard < 10 && cursorEnd > boundary.startMs; guard += 1) {
        ...
        if (page.capability === 'invalid-address') {
            throw new Error(`kline-invalid-address:${token.alphaId}:${dataType}`);
        }
        ...
        // The endpoint returns up to 1000 existing candles ending at endTime.
        // A short successful page proves there are no older candles available
        // for this token before oldestTs. This is critical for newly-listed
        // tokens such as CNPY: do not paginate into pre-listing time and turn
        // Binance's successful empty/missing-history envelope into a failure.
...
async function runTailsProducer(options) {
    ...
    logger.log(
        ...
        + `offline_bad_address=${Number(fullCohort.diagnostics?.offlineInvalidAddress || 0)}`,
    );
    ...
    return {
        ...
        offlineInvalidAddressCount: Number(fullCohort.diagnostics?.offlineInvalidAddress || 0),
