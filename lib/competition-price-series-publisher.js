...
function exactSnapshot(cache, scopeIds = null) {
    const data = {};
    const scope = normalizeScopeIds(scopeIds);
...
function createCompetitionPriceSeriesPublisher(options = {}) {
    ...
    async function publishSnapshot(cache, options = {}) {
        ...
        const scopeIds = normalizeScopeIds(options.scopeIds);
        const data = exactSnapshot(cache, scopeIds);
        if (scopeIds.length && Object.keys(data).length !== scopeIds.length) {
            throw new Error('Competition Price scoped publish missing requested series');
        }

        const mode = scopeIds.length ? 'merge' : 'replace';
        ...
        const body = JSON.stringify({
            schema: 1,
            ...(scopeIds.length ? { mode: 'merge', scopeIds } : {}),
