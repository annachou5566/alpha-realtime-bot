...
function getCompetitionMultipliers(config) {
    if (Array.isArray(config && config.multipliers) && config.multipliers.length) {
        return config.multipliers.map(value => Math.max(0, Number(value) || 1));
    }
    let earlyBird = config && config.earlyBird;
    if (earlyBird === true || earlyBird === 'true') earlyBird = '1.4x';
    if (earlyBird === '2.5x') return [2.5, 2.2, 1.8, 1.6, 1.4, 1.2, 1.0];
    if (earlyBird === '2.0_alt') return [2.0, 1.8, 1.8, 1.6, 1.4, 1.2, 1.0];
    if (earlyBird === '2.0x') return [2.0, 1.8, 1.7, 1.5, 1.3, 1.1, 1.0];
    if (earlyBird === '1.8x') return [1.8, 1.6, 1.4, 1.3, 1.3, 1.1, 1.0];
    if (earlyBird === '1.4x') return [1.4, 1.3, 1.2, 1.2, 1.1, 1.1, 1.0];

    const { startAt, endAt } = competitionWindow(config);
    if (!Number.isFinite(startAt) || !Number.isFinite(endAt) || endAt <= startAt) return [];
    const count = Math.max(1, Math.min(MAX_BUCKETS, Math.ceil((endAt - startAt) / DAY_MS)));
