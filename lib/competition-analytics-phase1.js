...
const ANALYTICS_METHOD = 'anchored-hourly-quote-volume-vwap-v3';
...
function parseRewardAt(config) {
    const date = String(config?.end || '').slice(0, 10);
...
function historyInterval(rewardAt) {
    return rewardAt % HOUR_MS === 0
        ? { interval: '1h', stepMs: HOUR_MS, candlesPerWindow: 1 }
        : { interval: '30m', stepMs: HALF_HOUR_MS, candlesPerWindow: 2 };
}

function anchoredHourlyPoints(rows, rewardAt, completedThrough, candlesPerWindow) {
    return aggregateHourlyVwap(rows, rewardAt).filter(point =>
...
function hourlyVwapPoints(rows, rewardAt, completedThrough) {
    return anchoredHourlyPoints(rows, rewardAt, completedThrough, 1);
}
...
async function fetchClaimVwap(alphaId, rewardAt) {
    const discovery = await fetchAlphaKlines(alphaId, '1m', {
        startTime: rewardAt, endTime: rewardAt + CLAIM_SEARCH_MS - 1, limit: KLINE_PAGE_SIZE,
    });
    const firstTrade = discovery.find(row => row.volume > 0);
    if (!firstTrade) return null;
    const rows = await fetchAlphaKlines(alphaId, '1m', {
        startTime: firstTrade.timestamp, endTime: firstTrade.timestamp + CLAIM_WINDOW_MS - 1, limit: 10,
    });
    ...
    return vwap && selected.length === 5 ? { vwap, observedAt: firstTrade.timestamp, candleCount: 5 } : null;
}
...
function recordFromTournament(row, previous = {}) {
    ...
    return {
        ...
        rewardAt: parseRewardAt(config), rewardUnit: reward.unit, rewardQty: reward.quantity,
...
async function loadEndedTournaments(supabase) {
    ...
    return (data || []).filter(row => {
        const rewardAt = parseRewardAt(row.data || {});
        const reward = rewardMeta(row.data || {}, row.name);
        return Number.isFinite(rewardAt) && rewardAt < now - CLAIM_WINDOW_MS && row.data?.alphaId && reward.unit && reward.unit !== 'USD' && reward.quantity;
    });
}
...
function resetHistoryState(record) {
    ...
    ['peakVwap','peakVwapAt','lowVwap','lowVwapAt','nextHistoryStartAt','completeThroughAt'].forEach(key => delete record[key]);
...
async function syncTournament(record, now = Date.now()) {
    ...
    if (!record.claimVwap || Number(record.claimCandleCount) !== 5) {
        ...
        if (claim) Object.assign(record, { claimVwap: claim.vwap, claimObservedAt: claim.observedAt, claimCandleCount: claim.candleCount });
    }
    ...
    const history = historyInterval(record.rewardAt);
    let cursor = Math.max(Number(record.nextHistoryStartAt || record.rewardAt), record.rewardAt);
    for (let page = 0; page < MAX_HISTORY_PAGES_PER_TOURNAMENT && cursor < completedThrough; page += 1) {
        ...
        updateExtremes(record, anchoredHourlyPoints(rows, record.rewardAt, completedThrough, history.candlesPerWindow));
    ...
    const ready = Number(record.claimCandleCount) === 5 && positive(record.claimVwap) && positive(record.currentPrice)
...
async function runCompetitionAnalyticsPhase1(options = {}) {
    ...
    return {
        ...
        ready: Object.values(state.tournaments).filter(row => row?.status === 'ready').length,
        bytes, results,
    };
}
...
module.exports = {
    MINUTE_MS, HALF_HOUR_MS, HOUR_MS, CLAIM_WINDOW_MS, STATE_KEY, ANALYTICS_METHOD,
    ...
    runCompetitionAnalyticsPhase1, startCompetitionAnalyticsPhase1,
};
