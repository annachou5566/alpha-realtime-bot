...
function createClients(env = process.env) {
    const supabaseReadKey = env.SUPABASE_ANON_KEY || env.SUPABASE_SERVICE_ROLE_KEY;
    const required = ['SUPABASE_URL', 'R2_ENDPOINT_URL', 'R2_ACCESS_KEY_ID', 'R2_SECRET_ACCESS_KEY', 'R2_BUCKET_NAME'];
    const missing = required.filter(name => !env[name]);
    if (!supabaseReadKey) missing.push('SUPABASE_ANON_KEY|SUPABASE_SERVICE_ROLE_KEY');
    if (missing.length) throw new Error(`Competition analytics missing env: ${missing.join(', ')}`);
    return {
        supabase: createClient(env.SUPABASE_URL, supabaseReadKey),
        r2: new S3Client({
